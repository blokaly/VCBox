...
async function getPoolGenesisTxnPath(poolName) {
  let path = `${os.tmpdir()}/indy-${getCurrentTimeInSeconds()}/${poolName}.txn`;
...
function poolConfigExists(file) {
    let path = pathToIndyClientHome() + "/pool/" + file
    return fs.existsSync(path)
}

function walletExists(name) {
  let path = pathToIndyClientHome() + "/wallet/" + name
  return fs.existsSync(path)
}
